...
根据前端开发规范，每次修改文件后需要确保：

1. 文件编码必须为UTF-8 ：所有文件都应该使用UTF-8编码保存，避免GBK/GB2312等编码导致乱码
2. 检查浏览器控制台 ：修改代码后应该在浏览器控制台检查是否有Vue模板编译错误或其他报错
3. 中文内容处理 ：确保所有中文文本在JavaScript字符串和Vue模板中正确显示
1. 每次更改之后，自动帮我重启程序 
2. 去掉多余的空行 
3. 禁止修改文件编码，你又给ui搞乱码了
