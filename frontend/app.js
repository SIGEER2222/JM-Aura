...
const BookCard = {
    ...
    <div class="md-card overflow-hidden cursor-pointer group flex flex-col h-full transition-all duration-300"
        ...
        <div class="p-3 md:p-4 flex flex-col flex-1">
            ...
            <button type="button" @click.stop="handleDislike"

                class="mt-3 h-9 w-full rounded-full bg-surface-container text-on-surface-variant hover:bg-error-container hover:text-on-error-container transition text-xs font-bold flex items-center justify-center gap-1">
...
createApp({
    ...
    data() {
        ...
        return {
            ...
            uiPreferences: {
                ...
                hideViewedComics: false,

                hideReadHistory: false
    ...
    computed: {
        ...
        jmFavFilteredItems() {
            ...
            const list = this.filterDislikedItems(this.jmFavItems);
        ...
        dislikedComicCount() {
            ...
            const m = (this.dislikedComicIds && typeof this.dislikedComicIds === 'object') ? this.dislikedComicIds : {};
        ...
        blockedTagCount() {
            ...
            const m = (this.blockedTags && typeof this.blockedTags === 'object') ? this.blockedTags : {};
        ...
        visibleHomeData() {
            ...
            return this.filterViewedHomeData(this.homeData);

        },

        visibleSearchResults() {

            return this.filterViewedItems(this.searchResults);

        },

        visibleFavorites() {

            return this.filterDislikedItems(this.favorites);

        },

        visibleJmLatestItems() {

            return this.filterViewedItems(this.jmLatestItems);

        },

        visibleJmLeaderboardItems() {

            return this.filterViewedItems(this.jmLeaderboardItems);

        },

        visibleJmCategoryItems() {

            return this.filterViewedItems(this.jmCategoryItems);

        },

        visibleJmRandomPreview() {

            const item = this.jmRandomPreview;

            if (!item) return null;

            return this.isComicFiltered(item) ? null : item;

        },

        visibleAlsoViewedItems() {

            return this.filterViewedItems(this.alsoViewedItems);

        },

        visibleJmHistoryItems() {

            return this.filterDislikedItems(this.jmHistoryItems);
        ...
        visibleJmLocalHistoryItems() {

            return this.filterDislikedItems(this.jmLocalHistoryItems);
    ...
    mounted() {
        ...
        this.initClientScope().finally(() => {
            ...
            this.initUiPreferences();

            this.loadReadingHistory();

            this.loadDislikedComics();

            this.loadBlockedTags();
    ...
    watch: {
        ...
        readerSettings: {
            ...
            handler() {
                ...
                if (!this.readerSettingsReady) return;

                this.persistReaderSettings();
    ...
    methods: {
        ...
        async onReaderImageFinished(imgIndex) {

            if (!this.readingChapter || !Array.isArray(this.readingChapter.images)) return;

            const idx = Math.max(0, parseInt(imgIndex || 0));

            // 获取真实总数（可能是分页返回的total_images）
            const realTotal = parseInt(this.readingChapter._total_images) || this.readingChapter.images.length || 0;
            const currentLoaded = this.readingChapter.images.length || 0;
            ...
            // 检查是否需要从服务器加载更多图片
            if (currentLoaded < realTotal && currentLoaded <= (this.readerLoadLimit || 0)) {
                // 需要加载更多图片
                try {
                    const photoId = this.readingChapter.photo_id;
                    const res = await fetch(`/api/chapter/${photoId}?limit=${batch}&offset=${currentLoaded}`);
                    if (res.ok) {
                        const moreData = await res.json();
                        if (moreData.images && Array.isArray(moreData.images)) {
                            // 追加新图片
                            this.readingChapter.images.push(...moreData.images);
                        }
                    }
                } catch (e) {
                    console.warn('Failed to load more images:', e);
                }
            }
        ...
        initUiPreferences() {
            ...
            try {
                ...
                const raw = localStorage.getItem(this.getStorageKey('uiPreferences'));
                ...
                if (raw) {
                    ...
                    if (v && typeof v === 'object') {
                        ...
                        this.uiPreferences = {
                            ...
                            hideViewedComics: !!v.hideViewedComics
        ...
        loadDislikedComics() {
            ...
            try {
                ...
                const raw = localStorage.getItem(this.getStorageKey('dislikedComicIds'));

                if (raw) {

                    const v = JSON.parse(raw);

                    this.dislikedComicIds = (v && typeof v === 'object') ? v : {};

                } else {

                    this.dislikedComicIds = {};
        ...
        saveDislikedComics() {
            ...
            try {
                ...
                localStorage.setItem(this.getStorageKey('dislikedComicIds'), JSON.stringify(this.dislikedComicIds || {}));
        ...
        loadBlockedTags() {
            ...
            try {
                ...
                const raw = localStorage.getItem(this.getStorageKey('blockedTags'));

                if (raw) {

                    const v = JSON.parse(raw);

                    this.blockedTags = (v && typeof v === 'object') ? v : {};
        ...
        normalizeReaderSettings(raw) {
            ...
            return {
                ...
                width: clampInt(src.width, 100, 20, 100),

                gap: clampInt(src.gap, 0, 0, 50),

                initial: clampInt(src.initial, 4, 1, 12),

                batch: clampInt(src.batch, 3, 1, 6)
        ...
        initReaderSettings() {
            ...
            try {
                ...
                for (const key of ['display_settings', 'readerSettings']) {
            ...
            try {
                ...
                this.readerSettings = this.normalizeReaderSettings(loaded || this.readerSettings);
        ...
        persistReaderSettings() {
            ...
            try {
                ...
                localStorage.setItem('display_settings', JSON.stringify(this.normalizeReaderSettings(this.readerSettings)));
        ...
        persistUiPreferences() {
            ...
            try {
                ...
                localStorage.setItem(this.getStorageKey('uiPreferences'), JSON.stringify(this.uiPreferences || {}));
        ...
        toggleUiPreference(key) {
            ...
            if (!this.uiPreferences || typeof this.uiPreferences !== 'object') {

                this.uiPreferences = { hideViewedComics: false, hideReadHistory: false };

            }

            if (!(key in this.uiPreferences)) return;

            this.uiPreferences[key] = !this.uiPreferences[key];
        ...
        dislikeComic(item) {
            ...
            const id = this.getComicItemId(item);
            ...
            this.showToast('已加入不喜欢，后续会隐藏', 'success');

            if (this.currentTab === 'jm_random') {
        ...
        isTagBlocked(tag) {
            ...
            return !!(this.blockedTags && typeof this.blockedTags === 'object' && this.blockedTags[t]);
        ...
        blockTag(tag) {
            ...
            this.blockedTags = { ...(this.blockedTags || {}), [t]: true };

            this.saveBlockedTags();

            this.showToast(`已屏蔽tag: ${tag}`, 'success');
        ...
        unblockTag(tag) {
            ...
            const newTags = { ...(this.blockedTags || {}) };

            delete newTags[t];

            this.blockedTags = newTags;

            this.saveBlockedTags();

            this.showToast(`已取消屏蔽tag: ${tag}`, 'success');

        },

        clearBlockedTags() {

            if (!this.blockedTagCount) return;

            this.askConfirm('清空屏蔽的Tag', `确定要清空 ${this.blockedTagCount} 个屏蔽的tag吗？`, () => {

                this.blockedTags = {};

                this.saveBlockedTags();

                this.showToast('已清空屏蔽的tag', 'success');
        ...
        saveBlockedTags() {
            ...
            try {
                ...
                localStorage.setItem(this.getStorageKey('blockedTags'), JSON.stringify(this.blockedTags || {}));
        ...
        addBlockedTag() {
            ...
            const tag = String(this.newBlockedTag || '').trim();

            if (!tag) {

                this.showToast('请输入要屏蔽的tag', 'error');
        ...
        clearDislikedComics() {
            ...
            if (!this.dislikedComicCount) return;

            this.askConfirm('清空不喜欢', `确定要清空 ${this.dislikedComicCount} 条不喜欢记录吗？`, () => {

                this.dislikedComicIds = {};

                this.saveDislikedComics();

                this.showToast('已清空不喜欢记录', 'success');
        ...
        getComicItemId(item) {
            ...
            return String(item.album_id || item.id || item.comic_id || item._id || '').trim();
        ...
        isComicViewed(item) {
            ...
            return !!(this.readingHistory && typeof this.readingHistory === 'object' && this.readingHistory[id]);

        },

        isComicDisliked(item) {

            const id = this.getComicItemId(item);

            if (!id) return false;

            return !!(this.dislikedComicIds && typeof this.dislikedComicIds === 'object' && this.dislikedComicIds[id]);

        },

        isComicFiltered(item) {

            if (this.isComicDisliked(item)) return true;

            if (this.isComicBlockedByTag(item)) return true;

            if (this.uiPreferences && this.uiPreferences.hideReadHistory && this.isComicViewed(item)) return true;

            return !!(this.uiPreferences && this.uiPreferences.hideViewedComics && this.isComicViewed(item));

        },

        isComicBlockedByTag(item) {
        ...
        filterViewedItems(items) {

            const list = Array.isArray(items) ? items : [];

            return list.filter(item => !this.isComicFiltered(item));

        },

        filterDislikedItems(items) {

            const list = Array.isArray(items) ? items : [];

            return list.filter(item => !this.isComicDisliked(item) && !this.isComicBlockedByTag(item));

        },

        filterViewedHomeData(data) {
            ...
            return list
                ...
                .map(category => {
                    ...
                    const content = this.filterViewedItems(category && category.content ? category.content : []);
        ...
        openComicPage(albumId) {
            ...
            url.searchParams.set('album_id', id);
        ...
        async openInitialComicFromUrl() {

            try {

                const url = new URL(window.location.href);

                const id = String(url.searchParams.get('album_id') || url.searchParams.get('comic_id') || '').trim();
        ...
        async loadJmRandomPreview() {
            ...
            try {
                ...
                for (let i = 0; i < maxAttempts; i++) {
                    ...
                    if (!this.isComicFiltered(picked)) {
        ...
        async loadAlsoViewed(comicId, force = false) {
            ...
            try {
                ...
                const res = await fetch(`/api/v2/jm/also_viewed/${encodeURIComponent(id)}?${params.toString()}`, {
                ...
                if (!res.ok || (json.st && json.st !== 1001)) {
                    ...
                    this.alsoViewedError = json.msg || json.detail || 'Failed';
                ...
                this.alsoViewedError = e && e.message ? e.message : 'Failed';
        ...
        async readChapter(photoId, title) {
            ...
            try {
                ...
                const res = await fetch(`/api/chapter/${photoId}?limit=${initialLimit}&offset=0`);
                ...
                try {
                    // 使用total_images获取真实总数，如果没有则使用当前images长度
                    const total = parseInt(this.readingChapter.total_images) || 
                                 ((this.readingChapter && Array.isArray(this.readingChapter.images)) ? this.readingChapter.images.length : 0);
                    const ini = Math.max(1, parseInt((this.readerSettings && this.readerSettings.initial) ? this.readerSettings.initial : 5));
                    ...
                    // 保存图片总数用于后续加载
                    if (this.readingChapter.total_images) {
                        this.readingChapter._total_images = this.readingChapter.total_images;
                    }
