...
# 全局缓存用于存储leaderboard结果
_leaderboard_cache: dict[str, dict] = {}
_chapter_cache: dict[str, dict] = {}
_cache_expire_time = timedelta(minutes=10)

def _get_cached_leaderboard(key: str) -> list | None:
    """获取缓存的leaderboard结果，如果过期返回None"""
    cached = _leaderboard_cache.get(key)
    if cached:
        if datetime.now() - cached['time'] < _cache_expire_time:
            return cached['data']
    return None

def _set_cached_leaderboard(key: str, data: list) -> None:
    """设置缓存的leaderboard结果"""
...
def _get_cached_chapter(chapter_id: str) -> dict | None:
    """获取缓存的章节详情，如果过期返回None"""
    cached = _chapter_cache.get(chapter_id)
    if cached:
        if datetime.now() - cached['time'] < _cache_expire_time:
            return cached['data']
    return None

def _set_cached_chapter(chapter_id: str, data: dict) -> None:
    """设置缓存的章节详情"""
    _chapter_cache[chapter_id] = {
        'data': data,
        'time': datetime.now()
    }
...
class JmProvider(ComicProvider):
    ...
    def categories(self) -> list[dict[str, Any]]:
        ...
        if isinstance(raw, dict):
            ...
            cats = raw.get("categories") or raw.get("data") or []
            ...
            for cat in cats:
                if isinstance(cat, dict):
                    # 检查slug字段
                    slug = str(cat.get("slug") or cat.get("SLUG") or "").lower()
                    # 检查title字段
                    title = str(cat.get("title") or cat.get("name") or "").lower()
                    # 如果是韩漫，跳过
                    if slug == "hanman" or "韩漫" in title:
                        continue
                filtered.append(cat)
    ...
    def leaderboard(self, **kwargs: Any) -> list[ComicSummary]:
        ...
        # 构建缓存key
        cache_key = f"{category}_{page}_{sort}_{tag or ''}"
        
        # 先检查缓存
        cached_items = _get_cached_leaderboard(cache_key)
        ...
        for it in items or []:
            if isinstance(it, dict) and it.get("album_id"):
                # 过滤掉韩漫类别
                comic_category = str(it.get("category") or "")
                if comic_category == "韩漫":
                    continue
        ...
        # 缓存结果
        _set_cached_leaderboard(cache_key, out)
    ...
    def chapter_detail(self, chapter_id: str, **kwargs: Any) -> ChapterDetail:
        # 尝试从缓存获取章节详情
        cached_data = _get_cached_chapter(chapter_id)
        if cached_data is not None:
            data = cached_data
        else:
            data = jm_service.get_chapter_detail(chapter_id)
            # 缓存章节详情
            _set_cached_chapter(chapter_id, data)
