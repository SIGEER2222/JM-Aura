...
def _extract_tags(data: dict) -> list[str]:
        ...
        if isinstance(raw_tags, list):
            for tag in raw_tags:
                if isinstance(tag, dict):
                    text = str(tag.get("title") or tag.get("name") or tag.get("tag") or "").strip()
                else:
    ...
    return [x for x in tags if not (x.casefold() in seen_tags or seen_tags.add(x.casefold()))]
    ...
    for key in ("tags", "tag", "tag_list", "tags_list"):
        ...
        if isinstance(raw_tags, list):
            ...
            if text:
                tags.append(text)
    seen_tags: set[str] = set()
    tags = [x for x in tags if not (x.casefold() in seen_tags or seen_tags.add(x.casefold()))]

    category = ""
    cat_obj = data.get("category") or data.get("categories") or data.get("category_list")
    if isinstance(cat_obj, list) and cat_obj:
        first = cat_obj[0]
        if isinstance(first, dict):
            category = str(first.get("title") or first.get("name") or first.get("slug") or "").strip()
        else:
            category = str(first or "").strip()
    elif isinstance(cat_obj, dict):
        category = str(cat_obj.get("title") or cat_obj.get("name") or cat_obj.get("slug") or "").strip()
    elif cat_obj is not None:
        category = str(cat_obj).strip()

    uploader = ""
    for key in ("uploader", "uploader_name", "upload_by", "uploaderName", "upload"):
        val = data.get(key)
        if val:
            uploader = str(val).strip()
            if uploader:
                break
